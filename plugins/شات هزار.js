...
if (/^غرانولا عمك$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, ` اتفق غوكو ❤️`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
}
   
   if (/^غابيمارو$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `الحزين البأس 🥺💔`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
   }
    if (/^سون غوكو$/i.test(m.text) ) { //sem prefixo 
 ...
     conn.reply(m.chat, ` قول نعم يا حب 🫶🏻 🐦❤`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
   ...
   if (/^غوكوبلاك$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'عمو الشرير 🥺',m) //wm, null, [['Menu', '#menu']], m) botones :v

   }

   if (/^كوكو|غرانولا|ايتاتشي|غرانولا|دراغون بول|مادارا|كريستيانو|فيجيتا|غوجيتا|فيجيتو $/i.test(m.text) ) { //sem prefixo
 ...
     conn.reply(m.chat, `اوهايو كوزيماس `, m) //wm, null, [['Menu', '#menu']], m) botones :V
   ...
   if (/^مادارا|فيجيتا|غوهان|كايدو|كروكودايل  $/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'صديق غرانولا الروح بروح 😂❤', m) //wm, null, [['Menu', '#menu']], m) botones :V
 ...
 if (/^ فلسطين|اخرائيل|اسرائيل$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, ` فلسطين حرة 🇵🇸`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
